CLASS zcljs_cust_cname_query DEFINITION PUBLIC FINAL CREATE PUBLIC .

  PUBLIC SECTION.
    INTERFACES:
      if_rap_query_provider.

    DATA: lv_url TYPE string VALUE 'https://685bd85e-501d-43dd-b091-aa6811254f67.abap-web.us10.hana.ondemand.com/sap/opu/odata/sap/ZUJS_COMPANY_NAME_V2_API/'.

  PROTECTED SECTION.
  PRIVATE SECTION.
    TYPES:
    tt_result TYPE STANDARD TABLE OF ZCJS_I_RAPCustomEntityCNames WITH EMPTY KEY.

    CONSTANTS:
      c_destination TYPE string VALUE 'Northwind_Odata_Service_V2_2', " 'destination-service-id',  "Northwind_Odata_Service_V2
...
CLASS ZCLJS_CUST_CNAME_QUERY IMPLEMENTATION.


  METHOD get_proxy.
    TRY.
        DATA(lo_destination) = cl_http_destination_provider=>create_by_cloud_destination( i_name = c_destination i_authn_mode = if_a4c_cp_service=>service_specific ).
