CLASS lhc_zr_djs_lang DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    METHODS:
...
CLASS lhc_zr_djs_lang IMPLEMENTATION.

  METHOD get_global_authorizations.
  ENDMETHOD.

  METHOD TranslateText.
    READ ENTITIES OF ZR_DJS_Lang IN LOCAL MODE
    ...
    LOOP AT lt_selected INTO DATA(ls_selected) WHERE TargetLanguage IS NOT INITIAL.
      DATA(ld_translated) = NEW zcljs_google_integration( )->translate_text(
          id_target_language = to_lower( ls_selected-TargetLanguage )
          id_text            = CONV #( ls_selected-SourceText ) ).

      MODIFY ENTITIES OF ZR_DJS_Lang IN LOCAL MODE
