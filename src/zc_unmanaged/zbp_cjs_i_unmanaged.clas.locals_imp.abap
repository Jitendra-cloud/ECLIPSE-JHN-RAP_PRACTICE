CLASS lcl_data_buffer DEFINITION.
  PUBLIC SECTION.
    CLASS-DATA gt_create TYPE zifjs_rap_data_handler=>tt_data.
    CLASS-DATA gt_update TYPE zifjs_rap_data_handler=>tt_data.
    CLASS-DATA gt_delete TYPE zifjs_rap_data_handler=>tt_data.
ENDCLASS.
...
CLASS lhc_Unmanaged IMPLEMENTATION.
  ...
  METHOD create.
    INSERT LINES OF
           CORRESPONDING zifjs_rap_data_handler=>tt_data( entities MAPPING cdate = CreationDate text = Description )
           INTO TABLE lcl_data_buffer=>gt_create.
  ENDMETHOD.


  METHOD update.
    DATA(lo_data_handler) = zcljs_rap_data_handler=>create_instance( ).
  ...
  METHOD delete.
    INSERT LINES OF
           CORRESPONDING zifjs_rap_data_handler=>tt_data( keys MAPPING gen_key = TableKey )
...
CLASS lsc_ZCJS_I_UNMANAGED DEFINITION INHERITING FROM cl_abap_behavior_saver.
   PROTECTED SECTION.
     METHODS
 ...
 CLASS lsc_ZCJS_I_UNMANAGED IMPLEMENTATION.
   METHOD finalize.
   ENDMETHOD.
