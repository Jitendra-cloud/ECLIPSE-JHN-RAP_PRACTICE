CLASS lcl_buffer DEFINITION.
  PUBLIC SECTION.
    TYPES tt_keys TYPE TABLE FOR ACTION IMPORT zcjs_r_currency\\currency~loadexcelcontent.
...
CLASS lhc_Currency IMPLEMENTATION.
  ...
  METHOD LoadExcelContent.
    ...
    "----------------------------------------------------------------------------------------------------------------------d5----
    DATA lt_currency_modify  TYPE TABLE FOR UPDATE ZCJS_R_Currency\\Currency.
    DATA lt_countries_create TYPE TABLE FOR CREATE ZCJS_R_Currency\_Country.
    DATA lt_countries_modify TYPE TABLE FOR UPDATE ZCJS_R_Currency\\Country.
    "----------------------------------------------------------------------------------------------------------------------d5----
    "----------------------------------------------------------------------------------------------------------------------d1----
    READ ENTITIES OF ZCJS_R_Currency IN LOCAL MODE
    ...
    TRY.
      ...
      CATCH cx_sy_itab_line_not_found.
        INSERT new_message( id       = 'ZMJS__RAP_PATTERN'
                            number   = '003'
    ...
    "----------------------------------------------------------------------------------------------------------------------d7----
    "----------------------------------------------------------------------------------------------------------------------d8----
    MODIFY ENTITIES OF ZCJS_R_Currency IN LOCAL MODE
...
CLASS lsc_ZCJS_R_CURRENCY DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZCJS_R_CURRENCY IMPLEMENTATION.
  ...
  METHOD save_modified.
    ...
    LOOP AT update-currency INTO DATA(ls_new_currency).
      ...
      INSERT zdjs_addcurr FROM @ls_new_currency MAPPING FROM ENTITY.
      IF sy-subrc <> 0.
        UPDATE zdjs_addcurr FROM @ls_new_currency INDICATORS SET STRUCTURE %control MAPPING FROM ENTITY.
      ENDIF.

    ENDLOOP.

    LOOP AT create-country INTO DATA(ls_create_country).
      INSERT zdjs_country FROM @ls_create_country MAPPING FROM ENTITY.
    ENDLOOP.

    LOOP AT update-country INTO DATA(ls_update_country).
      UPDATE zdjs_country FROM @ls_update_country INDICATORS SET STRUCTURE %control MAPPING FROM ENTITY.
    ENDLOOP.

    LOOP AT delete-country INTO DATA(ls_delete_country).
      DELETE zdjs_country FROM @( CORRESPONDING zdjs_country( ls_delete_country MAPPING FROM ENTITY ) ).
    ENDLOOP.

    LOOP AT lcl_buffer=>gt_event INTO DATA(ls_event).
      RAISE ENTITY EVENT ZCJS_R_Currency~AfterExcelLoad
